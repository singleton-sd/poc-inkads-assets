...
| `qr-go.svg` | Generated QR modules (260x260, >=4-module quiet zone) |
...
## Render / publish (Waveshare package)

```sh
pnpm render:house-ad
# or as part of the full catalog:
pnpm build
```

Uses `@singleton-sd/inkads-epaper-renderer@1.4.1` in **threshold** mode (preferred for
QR / text / logos). Outputs under `dist/marketing/house-ad/` (do not hand-edit):

| File | Description |
| --- | --- |
| `preview.png` | 800x480 1-bit preview of the packed buffer |
| `framebuffer.bin` | 48_000-byte packed Waveshare buffer |
| `metadata.json` | profileId, checksum, mode, rendererVersion, … |
| `house-ad-800x480.svg` / `qr-go.svg` | Published source copies |
...
| Asset | URL |
| --- | --- |
| Preview | `…/marketing/house-ad/preview.png` |
| Framebuffer | `…/marketing/house-ad/framebuffer.bin` |
| Metadata | `…/marketing/house-ad/metadata.json` |
| Master SVG | `…/marketing/house-ad/house-ad-800x480.svg` |

Firmware / device serving is out of scope for POC-260 (see other tracks).

## Venue distance / readability

- Designed for ~1–2 m hospitality viewing (lobby / corridor / F&B, not bathroom-only).
- CTA type >=20 px at 800x480; primary headline ~36 px.
- QR >=220 px with margin 4 modules — threshold mono render keeps modules crisp.
